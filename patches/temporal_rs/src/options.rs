...
use crate::{MS_PER_DAY, NS_PER_DAY, TemporalError, TemporalResult, error::ErrorMessage};
  ...
  pub(crate) fn resolve(&self) -> TemporalResult<ResolvedToStringRoundingOptions> {
    ...
    match self.smallest_unit {
      ...
      None => match self.precision {
        ...
        Precision::Digit(d) if (1..=3).contains(&d) => Ok(ResolvedToStringRoundingOptions {
          precision: Precision::Digit(d),
          smallest_unit: Unit::Millisecond,
          rounding_mode,
          increment: RoundingIncrement::try_new(10_u32.pow(3 - d as u32)).temporal_unwrap()?,
        }),
        Precision::Digit(d) if (4..=6).contains(&d) => Ok(ResolvedToStringRoundingOptions {
          precision: Precision::Digit(d),
          smallest_unit: Unit::Microsecond,
          rounding_mode,
          increment: RoundingIncrement::try_new(10_u32.pow(6 - d as u32)).temporal_unwrap()?,
        }),
        Precision::Digit(d) if (7..=9).contains(&d) => Ok(ResolvedToStringRoundingOptions {
          precision: Precision::Digit(d),
          smallest_unit: Unit::Nanosecond,
          rounding_mode,
          increment: RoundingIncrement::try_new(10_u32.pow(9 - d as u32)).temporal_unwrap()?,
        }),
        _ => Err(TemporalError::range().with_enum(ErrorMessage::FractionalDigitsPrecisionInvalid)),
      },
...
impl Unit {
  ...
  pub fn to_maximum_rounding_increment(self) -> Option<u32> {
    use Unit::{
      Auto, Day, Hour, Microsecond, Millisecond, Minute, Month, Nanosecond, Second, Week, Year,
    };
