...
use crate::{
  NS_PER_DAY, TemporalResult, TemporalUnwrap,
  builtins::core::{
...
use num_traits::{Euclid, cast::FromPrimitive};
...
impl IsoDateTime {
  ...
  pub fn new(date: IsoDate, time: IsoTime) -> TemporalResult<Self> {
    if !iso_dt_within_valid_limits(date, &time) {
      return Err(TemporalError::range().with_message("IsoDateTime not within a valid range."));
    }
...
impl IsoDate {
  ...
  pub fn check_validity(&self) -> TemporalResult<()> {
    if !self.is_valid() {
      return Err(TemporalError::range().with_message("IsoDateTime does not have valid fields."));
    }
    Ok(())
  }
  ...
  pub(crate) fn add_date_duration(
    ...
    let intermediate = Self::new_with_overflow(intermediate.0, intermediate.1, self.day, overflow)?;
  ...
  pub(crate) fn diff_iso_date(
    ...
    if largest_unit == Unit::Year || largest_unit == Unit::Month {
      ...
      while !iso_date_surpasses(
        ...
        intermediate =
          balance_iso_year_month(intermediate.0, i32::from(intermediate.1) + i32::from(sign));
      }
    ...
    let intermediate = balance_iso_year_month(self.year + years, i32::from(self.month) + months);
    ...
    let days = utils::epoch_days_from_gregorian_date(other.year, other.month, other.day)
      - utils::epoch_days_from_gregorian_date(constrained.year, constrained.month, constrained.day);
...
impl IsoTime {
  ...
  pub(crate) fn round(
    ...
    let quantity = match resolved_options.smallest_unit {
      ...
      _ => {
        return Err(
          TemporalError::range().with_message("Invalid smallestUnit value for time rounding."),
        );
      }
    };
  ...
  pub(crate) fn to_epoch_ms(self) -> i64 {
    ((i64::from(self.hour) * utils::MS_PER_HOUR + i64::from(self.minute) * utils::MS_PER_MINUTE)
...
#[cfg(test)]
mod tests {
  use super::{IsoDate, iso_date_to_epoch_days};
