...
use writeable::{LengthHint, Writeable, impl_display_with_writeable};
...
impl Writeable for FormattableYearMonth<'_> {
  ...
  fn writeable_length_hint(&self) -> LengthHint {
    ...
    let base_length = self.calendar.writeable_length_hint() + LengthHint::exact(year_length + 3);
    if self.calendar.show == DisplayCalendar::Always
      || self.calendar.show == DisplayCalendar::Critical
      || self.calendar.calendar != "iso8601"
    {
...
fn parse_ixdtf(source: &[u8], variant: ParseVariant) -> TemporalResult<IxdtfParseRecord<'_, Utf8>> {
  ...
  ))) = record.offset
  {
    if fraction.to_nanoseconds().is_none() {
      return Err(TemporalError::range().with_enum(ErrorMessage::FractionalTimeMoreThanNineDigits));
    }
  }
  ...
  if critical_duplicate_calendar {
    ...
    return Err(
      TemporalError::range().with_message("Duplicate calendar value with critical flag found."),
    );
  }

  // Validate that the DateRecord exists.
  if variant != ParseVariant::Time && record.date.is_none() {
    return Err(TemporalError::range().with_message("DateTime strings must contain a Date value."));
  }
  ...
  if let Some(date) = record.date {
    ...
    if !crate::iso::is_valid_date(year, date.month, day) {
      return Err(
        TemporalError::range().with_message("DateTime strings must contain a valid ISO date."),
      );
    }
  }
...
pub(crate) fn parse_date_time(source: &[u8]) -> TemporalResult<IxdtfParseRecord<'_, Utf8>> {
  ...
  if record.offset == Some(UtcOffsetRecordOrZ::Z) {
    return Err(
      TemporalError::range().with_message("UTC designator is not valid for DateTime parsing."),
    );
  }
