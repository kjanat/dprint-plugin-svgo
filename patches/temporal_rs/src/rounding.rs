...
impl<T: Roundable> IncrementRounder<T> {
  ...
  pub fn round(&self, mode: RoundingMode) -> i128 {
    ...
    let mut rounded = apply_unsigned_rounding_mode(dividend, self.divisor, unsigned_rounding_mode);
    if !self.sign {
      rounded = rounded.neg();
    }
  ...
  pub fn round_as_if_positive(&self, mode: RoundingMode) -> i128 {
    ...
    let rounded = apply_unsigned_rounding_mode(self.dividend, self.divisor, unsigned_rounding_mode);
...
impl Roundable for f64 {
  ...
  fn is_even_cardinal(dividend: Self, divisor: Self) -> bool {
    let quotient = dividend / divisor;
    (FloatCore::floor(quotient) / (FloatCore::ceil(quotient) - FloatCore::floor(quotient)) % 2.0)
      == 0.0
  }
...
mod tests {
  ...
  impl<T: Roundable + Debug> TestCase<T> {
    fn run(&self) {
      let rounder =
        IncrementRounder::from_signed_num(self.x, TryFrom::try_from(self.increment).unwrap())
          .unwrap();
