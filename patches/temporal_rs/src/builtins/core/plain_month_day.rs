...
mod tests {
  ...
  fn test_to_plain_date_with_year() {
    let month_day =
      PlainMonthDay::new_with_overflow(5, 15, Calendar::default(), Overflow::Reject, None).unwrap();
  ...
  fn test_to_plain_date_with_era_and_era_year() {
    ...
    let month_day =
      PlainMonthDay::new_with_overflow(3, 10, calendar.clone(), Overflow::Reject, None).unwrap();
  ...
  fn test_to_plain_date_missing_year_and_era() {
    let month_day =
      PlainMonthDay::new_with_overflow(7, 4, Calendar::default(), Overflow::Reject, None).unwrap();
  ...
  fn test_to_plain_date_with_fallback_logic_matches_date() {
    ...
    let calendar = Calendar::from_str("gregory").unwrap();
    let month_day =
      PlainMonthDay::new_with_overflow(12, 25, calendar.clone(), Overflow::Reject, None).unwrap();
