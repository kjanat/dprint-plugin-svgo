...
use crate::{
  Calendar, NS_PER_DAY, NS_PER_DAY_NONZERO, TemporalError, TemporalResult, TemporalUnwrap,
  builtins::{
    core::{PlainDate, PlainDateTime, time_zone::TimeZone},
    duration::TWO_POWER_FIFTY_THREE,
  },
  iso::{IsoDate, IsoDateTime},
  options::{
    Disambiguation, Overflow, ResolvedRoundingOptions, RoundingIncrement, RoundingMode,
    UNIT_VALUE_TABLE, Unit,
  },
...
impl TimeDuration {
  ...
  pub(crate) fn from_duration(duration: &Duration) -> Self {
    ...
    let mut nanoseconds: i128 = i128::from(duration.hours) * NANOSECONDS_PER_HOUR * sign_multiplier;
  ...
  pub(crate) fn from_nanosecond_difference(one: i128, two: i128) -> TemporalResult<Self> {
    ...
    if result.abs() > MAX_TIME_DURATION {
      return Err(TemporalError::range().with_message("TimeDuration exceeds maxTimeDuration."));
    }
    Ok(Self(result))
  }
...
impl InternalDurationRecord {
  ...
  fn nudge_calendar_unit(
    ...
    let (r1, r2, start_duration, end_duration) = match options.smallest_unit {
      ...
      Unit::Year => {
        ...
        let r2 =
          years + i128::from(options.increment.get()) * i128::from(sign.as_sign_multiplier());
      ...
      Unit::Month => {
        ...
        let r2 =
          months + i128::from(options.increment.get()) * i128::from(sign.as_sign_multiplier());
      ...
      Unit::Week => {
        ...
        let r2 =
          weeks + i128::from(options.increment.get()) * i128::from(sign.as_sign_multiplier());
      ...
      Unit::Day => {
        ...
        let r2 = days + i128::from(options.increment.get()) * i128::from(sign.as_sign_multiplier());
      ...
      _ => {
        ...
        return Err(
          TemporalError::assert().with_message("NudgeCalendarUnit invoked with unexpected unit"),
        );
      }
    };
    ...
    let total =
      r1 as f64 + progress * options.increment.get() as f64 * f64::from(sign.as_sign_multiplier());
  ...
  fn nudge_to_zoned_time(
    ...
    let end = time_zone.get_epoch_nanoseconds_for(end_dt, Disambiguation::Compatible, provider)?;
    ...
    let (expanded, day_delta, rounded_time, nudge_ns) = if beyond_day_span.sign() != sign.negate() {
  ...
  fn bubble_relative_duration(
    ...
    for unit in unit_values.iter().rev().copied() {
      ...
      if unit != Unit::Week || largest_unit == Unit::Week {
        ...
        let end = calendar.date_add(&iso_date_time.date, &end_duration, Overflow::Constrain)?;
  ...
  pub(crate) fn round_relative_duration(
    ...
    let nudge_result = if irregular_length_unit {
      ...
      duration.nudge_calendar_unit(sign, origin_epoch_ns, dest_epoch_ns, dt, time_zone, options)?
