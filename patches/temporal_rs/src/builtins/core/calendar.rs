...
impl Calendar {
  ...
  pub fn month_day_from_fields(
    ...
    if fields.year.is_some() || (fields.era.is_some() && fields.era_year.is_some()) {
      ...
      if self.is_iso() {
        let resolved =
          ResolvedIsoFields::try_from_fields(&fields, overflow, ResolutionType::MonthDayWithYear)?;
        fields = CalendarFields {
          year: Some(1972),
        ...
        let date_fields = DateFields::try_from(&fields)?;
        {
          ...
          if iso_min.check_within_limits().is_err() && iso_max.check_within_limits().is_err() {
