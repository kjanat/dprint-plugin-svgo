...
use crate::{NonZeroSign, provider::EpochNanosecondsAndOffset};
...
use crate::{
  ...
  options::{
    ...
    DisplayTimeZone, OffsetDisambiguation, Overflow, ResolvedRoundingOptions, RoundingIncrement,
    RoundingMode, RoundingOptions, ToStringRoundingOptions, Unit, UnitGroup,
  },
...
impl ZonedDateTime {
  ...
  pub(crate) fn add_zoned_date_time(
    ...
    if !intermediate.is_within_limits() {
      return Err(TemporalError::range().with_enum(ErrorMessage::IntermediateDateTimeOutOfRange));
    }
  ...
  match (is_exact, offset_nanos, offset_option) {
    ...
    (_, Some(offset), OffsetDisambiguation::Prefer | OffsetDisambiguation::Reject) => {
      ...
      if offset_option == OffsetDisambiguation::Reject {
        return Err(TemporalError::range().with_enum(ErrorMessage::OffsetNeedsDisambiguation));
      }
      // 12. Return ? DisambiguatePossibleEpochNanoseconds(possibleEpochNs, timeZone, isoDateTime, disambiguation).
      timezone.disambiguate_possible_epoch_nanos(possible_nanos, iso, disambiguation, provider)
    }
  }
}
