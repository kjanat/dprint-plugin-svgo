...
use crate::parsers::{
  FormattableOffset, FormattableTime, Precision, parse_allowed_timezone_formats, parse_identifier,
};
...
use crate::{
  ...
  builtins::core::{Instant, duration::normalized::TimeDuration},
  ...
  pub(crate) fn from_ixdtf_record(record: UtcOffsetRecord) -> TemporalResult<Self> {
    ...
    if let Some(second) = record.second() {
      ...
      if let Some(frac) = record.fraction() {
        ns += i64::from(frac.to_nanoseconds().ok_or(
  ...
  pub(crate) fn get_offset_nanos_for(
    ...
    match self {
      ...
      Self::IanaIdentifier(identifier) => {
        let offset =
          provider.transition_nanoseconds_for_utc_epoch_nanoseconds(*identifier, utc_epoch)?;
  ...
  pub(crate) fn get_possible_epoch_ns_for(
    ...
    let possible_nanoseconds = match self {
      ...
      Self::IanaIdentifier(identifier) => {
        ...
        // isoDateTime).
        provider.candidate_nanoseconds_for_local_epoch_nanoseconds(*identifier, local_iso.into())?
      }
    };
...
mod tests {
  ...
  fn test_possible_epoch_ns_at_limits_helper(
    ...
    use crate::provider::{CandidateEpochNanoseconds, EpochNanosecondsAndOffset, UtcOffsetSeconds};
