...
use crate::{
  ...
  error::ErrorMessage,
  iso::{IsoDate, IsoDateTime, IsoTime, year_month_within_limits},
  options::{
...
use super::{Duration, PlainDate, PlainDateTime, duration::normalized::InternalDurationRecord};
...
impl PlainYearMonth {
  ...
  pub(crate) fn add_duration(
    ...
    {
      return Err(
        TemporalError::range().with_message("Can only add years or months to PlainYearMonth."),
      );
    }
  ...
  pub(crate) fn diff(
    ...
    if self.calendar().identifier() != other.calendar().identifier() {
      return Err(
        TemporalError::range().with_message("Calendars for difference operation are not the same."),
      );
    }
    ...
    {
      return Err(
        TemporalError::range().with_message("Weeks and days are not allowed in this operation."),
      );
    }
    ...
    let other_fields = CalendarFields::from(YearMonthCalendarFields::try_from_year_month(other)?);
    ...
    if resolved.smallest_unit != Unit::Month || resolved.increment != RoundingIncrement::ONE {
      ...
      let target_iso_date_time = IsoDateTime::new_unchecked(other_date.iso, IsoTime::default());
...
impl PlainYearMonth {
  ...
  pub fn to_ixdtf_string(&self, display_calendar: DisplayCalendar) -> String {
    self
      .to_ixdtf_writeable(display_calendar)
      .write_to_string()
      .into()
  }
