  ...
  pub fn try_from_fields(
    ...
    let mut month = match (fields.month, fields.month_code) {
      ...
      (ordinal, Some(code)) => {
        ...
        if code.is_leap_month() {
          return Err(
            TemporalError::range().with_message("No leap months allowed for ISO calendar"),
          );
        }
        ...
        if let Some(ordinal) = ordinal {
          if ordinal != found_ordinal {
            return Err(TemporalError::range().with_message("Month does not match monthCode."));
          }
        }
        found_ordinal
      }
      (None, None) => {
        return Err(
          TemporalError::r#type().with_message("Required month/monthCode field is empty."),
        );
      }
    };
    ...
    let day = if overflow == Overflow::Constrain {
      ...
      if !is_valid_iso_day(arithmetic_year, month, day) {
        return Err(TemporalError::range().with_message("day value is not in a valid range."));
      }
      day
    };
    Ok(Self {
...
impl MonthCode {
  pub(crate) fn validate(&self, calendar: AnyCalendarKind) -> TemporalResult<()> {
    ...
    match calendar {
      AnyCalendarKind::Chinese | AnyCalendarKind::Dangi if LUNAR_LEAP_MONTHS.contains(&self.0) => {
        Ok(())
      }
      ...
      _ => Err(
        TemporalError::range().with_message("MonthCode was not valid for the current calendar."),
      ),
    }
  }
  ...
  pub fn try_from_utf8(src: &[u8]) -> TemporalResult<Self> {
    if !(3..=4).contains(&src.len()) {
      return Err(TemporalError::range().with_message("Month codes must have 3 or 4 characters."));
    }
    ...
    if bytes[0] != b'M' {
      return Err(TemporalError::range().with_message("First month code character must be 'M'."));
    }
...
mod tests {
  ...
  use crate::{
    builtins::{
      calendar::{CalendarFields, types::ResolutionType},
