...
use crate::{
  ...
  builtins::{
    ...
    core::{
      Duration, PlainDateTime, PlainTime, ZonedDateTime, calendar::Calendar, duration::DateDuration,
    },
  },
...
use super::{PlainMonthDay, PlainYearMonth, duration::normalized::InternalDurationRecord};
...
impl PlainDate {
  ...
  pub fn from_partial(partial: PartialDate, overflow: Option<Overflow>) -> TemporalResult<Self> {
    let year_check = partial.calendar_fields.year.is_some()
      || (partial.calendar_fields.era.is_some() && partial.calendar_fields.era_year.is_some());
    let month_check =
