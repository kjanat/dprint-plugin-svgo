...
use crate::{
  ...
  builtins::core::{Duration, zoned_date_time::nanoseconds_to_formattable_offset_minutes},
  error::ErrorMessage,
  iso::IsoDateTime,
  options::{
  ...
  parsers::{IxdtfStringBuilder, parse_instant},
...
impl Instant {
  ...
  pub(crate) fn round_instant(
    ...
    let increment = match resolved_options.smallest_unit {
      Unit::Hour => {
        increment.checked_mul(NonZeroU128::new(NANOSECONDS_PER_HOUR as u128).temporal_unwrap()?)
      }
      Unit::Minute => {
        increment.checked_mul(NonZeroU128::new(NANOSECONDS_PER_MINUTE as u128).temporal_unwrap()?)
      }
      Unit::Second => {
        increment.checked_mul(NonZeroU128::new(NANOSECONDS_PER_SECOND as u128).temporal_unwrap()?)
      }
      Unit::Millisecond => increment.checked_mul(NonZeroU128::new(1_000_000).temporal_unwrap()?),
      Unit::Microsecond => increment.checked_mul(NonZeroU128::new(1_000).temporal_unwrap()?),
      Unit::Nanosecond => Some(increment),
      _ => {
        return Err(
          TemporalError::range().with_message("Invalid unit provided for Instant::round."),
        );
      }
    };
...
impl Instant {
  ...
  pub fn to_ixdtf_string_with_provider(
    ...
    self
      .to_ixdtf_writeable_with_provider(timezone, options, provider)
  ...
  pub fn to_ixdtf_writeable_with_provider(
    ...
    let datetime = if let Some(timezone) = timezone {
      ...
      TimeZone::utc_with_provider(provider).get_iso_datetime_for(&rounded_instant, provider)?
    };
    let builder = ixdtf
...
mod tests {
  ...
  fn instant_add_across_epoch() {
    ...
    use crate::{options::ToStringRoundingOptions, partial::PartialDuration, tzdb::FsTzdbProvider};
    use core::str::FromStr;
  ...
  fn instant_add_subtract_max_min() {
    ...
    let zero =
      Duration::from_partial_duration(PartialDuration::default().with_nanoseconds(0)).unwrap();
    let one =
      Duration::from_partial_duration(PartialDuration::default().with_nanoseconds(1)).unwrap();
    let neg_one =
      Duration::from_partial_duration(PartialDuration::default().with_nanoseconds(-1)).unwrap();
