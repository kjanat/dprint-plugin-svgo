...
use crate::{
  NS_PER_DAY, Sign, TemporalError, TemporalResult,
...
impl Duration {
  ...
  pub fn from_partial_duration(partial: PartialDuration) -> TemporalResult<Self> {
    if partial == PartialDuration::default() {
      return Err(
        TemporalError::r#type().with_message("PartialDuration cannot have all empty fields."),
      );
    }
    Self::new(
  ...
  pub fn compare_with_provider(
    ...
    if let Some(RelativeTo::ZonedDateTime(zdt)) = relative_to.as_ref() {
      if largest_unit_1.is_date_unit() || largest_unit_2.is_date_unit() {
        ...
        let after1 = zdt.add_zoned_date_time(duration_one, Overflow::Constrain, provider)?;
    ...
    let (days1, days2) = if largest_unit_1.is_calendar_unit() || largest_unit_2.is_calendar_unit() {
...
impl Duration {
  ...
  pub fn add(&self, other: &Self) -> TemporalResult<Self> {
    ...
    if largest_unit.is_calendar_unit() {
      return Err(
        TemporalError::range()
          .with_message("Largest unit cannot be a calendar unit when adding two durations."),
      );
    }
  ...
  pub fn round_with_provider(
    ...
    if options.largest_unit.is_none() && options.smallest_unit.is_none() {
      ...
      return Err(
        TemporalError::range().with_message("smallestUnit and largestUnit cannot both be None."),
      );
    }
    ...
    if Unit::larger(largest_unit, smallest_unit)? != largest_unit {
      return Err(TemporalError::range().with_message("smallestUnit is larger than largestUnit."));
    }
    ...
    match relative_to {
      ...
      Some(RelativeTo::ZonedDateTime(zoned_relative_to)) => {
        ...
        let internal =
          zoned_relative_to.diff_with_rounding(&target_epoch_ns, resolved_options, provider)?;
      ...
      Some(RelativeTo::PlainDate(plain_relative_to)) => {
        ...
        let date_duration = internal_duration
          .date()
        ...
        let target_date =
          calendar.date_add(&plain_relative_to.iso, &date_duration, Overflow::Constrain)?;

        // f. Let isoDateTime be CombineISODateAndTimeRecord(plainRelativeTo.[[ISODate]], MidnightTimeRecord()).
        let iso_date_time = IsoDateTime::new_unchecked(plain_relative_to.iso, IsoTime::default());
        ...
        let internal_duration = PlainDateTime::new_unchecked(iso_date_time, calendar.clone())
    ...
    if existing_largest_unit.is_calendar_unit() || resolved_options.largest_unit.is_calendar_unit()
    {
      return Err(
        TemporalError::range()
          .with_message("largestUnit when rounding Duration was not the largest provided unit"),
      );
    }
    ...
    let internal_duration = if resolved_options.smallest_unit == Unit::Day {
      ...
      let days = internal_duration
        .normalized_time_duration()
        .round_to_fractional_days(resolved_options.increment, resolved_options.rounding_mode)?;
  ...
  pub fn total_with_provider(
    ...
    match relative_to {
      ...
      Some(RelativeTo::ZonedDateTime(zoned_datetime)) => {
        ...
        // e. Let targetEpochNs be ? AddZonedDateTime(relativeEpochNs, timeZone, calendar, internalDuration, constrain).
        let target_epoch_ns =
          zoned_datetime.add_zoned_date_time(internal_duration, Overflow::Constrain, provider)?;
      ...
      Some(RelativeTo::PlainDate(plain_date)) => {
        ...
        let target_date =
          plain_date
            .calendar()
            .date_add(&plain_date.iso, &date_duration, Overflow::Constrain)?;
        // f. Let isoDateTime be CombineISODateAndTimeRecord(plainRelativeTo.[[ISODate]], MidnightTimeRecord()).
        ...
        let plain_dt = PlainDateTime::new_unchecked(iso_date_time, plain_date.calendar().clone());
  ...
  pub fn as_temporal_string(&self, options: ToStringRoundingOptions) -> TemporalResult<String> {
    if options.smallest_unit == Some(Unit::Hour) || options.smallest_unit == Some(Unit::Minute) {
      return Err(
        TemporalError::range()
          .with_message("string rounding options cannot have hour or minute smallest unit."),
      );
    }
...
pub(crate) fn is_valid_duration(
  ...
  let total_normalized_seconds = normalized_nanoseconds.saturating_add(normalized_subseconds_parts);
