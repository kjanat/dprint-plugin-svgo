...
use crate::{
  ...
  builtins::{
    ...
    core::{Instant, calendar::Calendar},
  },
...
impl core::fmt::Display for PlainDateTime {
  fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
    let string = self.to_ixdtf_string(ToStringRoundingOptions::default(), DisplayCalendar::Auto);
...
impl PlainDateTime {
  ...
  pub fn from_partial(
    ...
    let iso_time = IsoTime::default().with(partial.fields.time, overflow.unwrap_or_default())?;
...
impl PlainDateTime {
  ...
  #[inline]
  pub fn subtract(&self, duration: &Duration, overflow: Option<Overflow>) -> TemporalResult<Self> {
  ...
  pub fn to_ixdtf_string(
    ...
    self
      .to_ixdtf_writeable(options, display_calendar)
...
mod tests {
  ...
  use crate::{
    ...
    builtins::{
      calendar::CalendarFields,
      core::{
        Duration, PartialTime, PlainDateTime, calendar::Calendar, duration::DateDuration,
        plain_date_time::DateTimeFields,
      },
    },
    iso::{IsoDate, IsoDateTime, IsoTime},
    options::{
  ...
  fn to_string_precision_digits() {
    ...
    let subseconds =
      PlainDateTime::try_new(1976, 11, 18, 15, 23, 30, 123, 400, 0, Calendar::default()).unwrap();
