...
impl ProcessPluginMessage {
  pub fn read<TRead: Read + Unpin>(
    reader: &mut MessageReader<TRead>,
  ) -> Result<ProcessPluginMessage> {
    ...
    let body = match message_kind {
      ...
      message_ids::RELEASE_CONFIG_ID => {
        MessageBody::ReleaseConfig(FormatConfigId::from_raw(reader.read_u32()?))
      }
      message_ids::GET_CONFIG_DIAGNOSTICS_ID => {
        MessageBody::GetConfigDiagnostics(FormatConfigId::from_raw(reader.read_u32()?))
      }
      message_ids::GET_FILE_MATCHING_INFO_ID => {
        MessageBody::GetFileMatchingInfo(FormatConfigId::from_raw(reader.read_u32()?))
      }
      message_ids::GET_RESOLVED_CONFIG_ID => {
        MessageBody::GetResolvedConfig(FormatConfigId::from_raw(reader.read_u32()?))
      }
      ...
      message_ids::FORMAT_ID => {
        ...
        let file_bytes = reader.read_sized_bytes()?;
        MessageBody::Format(FormatMessageBody {
          file_path: PathBuf::from(String::from_utf8_lossy(&file_path).to_string()),
          range: if start_byte_index == 0 && end_byte_index == file_bytes.len() as u32 {
            None
          } else {
            Some(std::ops::Range {
      ...
      message_ids::HOST_FORMAT_ID => MessageBody::HostFormat,
      _ => {
    ...
    match &self.body {
      ...
      MessageBody::Format(body) => {
        ...
        writer.send_u32(
          body
            ...
            .as_ref()
            .map(|r| r.end)
            .unwrap_or(body.file_bytes.len()) as u32,
        )?;
        writer.send_u32(body.config_id.as_raw())?;
      ...
      MessageBody::HostFormat => {
        return Err(std::io::Error::new(
          ...
          "HostFormat cannot be written by this plugin",
        ));
      }
      MessageBody::Unknown(_) => unreachable!(),
    }
...
pub type CheckConfigUpdatesMessageBody = CheckConfigUpdatesMessage;
